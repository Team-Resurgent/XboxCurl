...
bearssl_connect_step3(struct connectdata *conn, int sockindex)
{
  ...
  if(session_params.session_id_len > 0) {
    ...
    /* Note: BearSSL doesn't have a direct "was resumed" flag,
       but we can infer from session ID presence on a fresh context */
    infof(data, "BearSSL: %s connection using %s\n",
          BACKEND->session_resumed ? "Resumed" : "New", version_str);
  }
  else {
    infof(data, "BearSSL: New connection using %s\n", version_str);
  }
...
static void Curl_bearssl_close(struct connectdata *conn, int sockindex)
{
  ...
  /* Only cleanup if SSL was actually initialized on this connection */
  ...
  if(BACKEND) {
    ...
    /* Free per-connection file-loaded trust anchors before clearing */
#if BEARSSL_XBOX_INTEGRATION
    if(BACKEND->file_anchors) {
