...
namespace
{
	...
	int trust_anchor_count = 0;
	int loaded_from_file = 0; /* 1 = heap-allocated per-anchor data */
}
...
void certificates::close_trust_anchors()
{
	...
	if (loaded_from_file)
	{
		ca_loader::free_trust_anchors(trust_anchors, (size_t)trust_anchor_count);
	}
	else
	{
		free(trust_anchors);
...
{
	return trust_anchor_count;
}

int certificates::load_from_file(const char *path)
{
	br_x509_trust_anchor *file_anchors = NULL;
	size_t file_count = 0;

	if (ca_loader::load_pem_file(path, &file_anchors, &file_count) != 0)
	{
		return -1;
	}

	/* Release any existing trust anchors */
	close_trust_anchors();

	trust_anchors = file_anchors;
	trust_anchor_count = (int)file_count;
	loaded_from_file = 1;
	return 0;
}
