...
#ifdef _DEBUG
#define curl_debug(msg) OutputDebugStringA(msg)
#else
#define curl_debug(msg) ((void)0)
#endif
    ...
    const char *server_name)
{
  ...
  if(!xc->minimal || !xc->minimal->vtable) {
    curl_debug("BearSSL noanchor X509: ERROR - minimal or vtable is NULL!\n");
    return;
  }
  ...
  curl_debug("BearSSL noanchor X509: start_chain forwarded OK\n");
}
...
#ifdef _XBOX
        curl_debug("BearSSL noanchor X509: CERTIFICATE PIN MISMATCH - rejecting!\n");
#endif
...
#ifdef _XBOX
    curl_debug("BearSSL noanchor X509: ERROR - no valid key available!\n");
#endif
  }
...
#ifdef BEARSSL_VERBOSE_DEBUG
    curl_debug("BearSSL sock_read: connection closed by peer\n");
#endif
    return -1;
  }
...
bearssl_connect_step1(struct connectdata *conn, int sockindex)
{
  ...
  if(!SSL_CONN_CONFIG(verifypeer)) {
     ...
     */
    curl_debug("BearSSL: Certificate verification DISABLED - using noanchor validator\n");
    ...
    curl_debug("BearSSL: Calling br_ssl_client_init_full...\n");
    ...
    curl_debug("BearSSL: br_ssl_client_init_full done\n");
    ...
    curl_debug("BearSSL: Setting X509 engine to noanchor wrapper...\n");
    ...
    curl_debug("BearSSL: Noanchor X509 validator installed\n");
  }
  else {
#if BEARSSL_XBOX_INTEGRATION
    {
  ...
  curl_debug("BearSSL: Setting protocol versions...\n");
  ...
  curl_debug("BearSSL: ALPN protocols set (http/1.1)\n");

  /* Set I/O buffer */
  br_ssl_engine_set_buffer(&BACKEND->sc.eng, BACKEND->iobuf,
                           sizeof(BACKEND->iobuf), 1);
  curl_debug("BearSSL: I/O buffer set\n");
...
#ifdef _XBOX
  {
    ...
    if(conn->sock[sockindex] == CURL_SOCKET_BAD) {
      curl_debug("BearSSL step1: ERROR - socket is INVALID!\n");
    }
  }
#endif

  curl_debug("BearSSL: Calling br_sslio_init...\n");
  ...
  curl_debug("BearSSL: br_sslio_init done\n");

  /* Note: BACKEND->active is set in step3 after handshake completes */
  connssl->connecting_state = ssl_connect_2;

  curl_debug("BearSSL step1: complete, moving to step2\n");
  return CURLE_OK;
}
...
bearssl_connect_step2(struct connectdata *conn, int sockindex)
{
  ...
  if(state & BR_SSL_SENDREC) {
    /* Need to send data */
    int ret;
    curl_debug("BearSSL step2: flushing send buffer...\n");
  ...
  if(state & BR_SSL_RECVREC) {
    ...
    unsigned char tmp[1];
    int ret;
    curl_debug("BearSSL step2: attempting to receive...\n");
...
#ifdef _XBOX
      curl_debug("BearSSL: ALPN not negotiated by server\n");
#endif
    }
  }
...
#ifdef _XBOX
    curl_debug("BearSSL step3: Leaf cert hash not available (resumed session)\n");
#endif
  }
#endif
...
#ifdef _XBOX
  curl_debug("BearSSL: Registered send/recv handlers with libcurl\n");
#endif
                            ...
                            const void *mem, size_t len, CURLcode *curlcode)
{
  ...
  if(!BACKEND || !BACKEND->active) {
    curl_debug("bearssl_send: BACKEND invalid, returning error\n");
                            ...
                            char *buf, size_t buffersize, CURLcode *curlcode)
{
  ...
  /* Defensive check - BACKEND must be valid */
  if(!BACKEND || !BACKEND->active) {
    curl_debug("bearssl_recv: BACKEND invalid, returning error\n");
